...
def standardize(objective, constraints, types, goal):
    ...
    # For MIN: negate coeff to convert MIN→MAX, then apply -c_j convention → result is +c_j
    # For MAX: just apply -c_j convention
    orig_obj_row = 1 if has_phase1 else 0

    for j, c in enumerate(objective):
        if goal.lower() == "min":
            # For MIN: negate once to convert to MAX problem
            tableau[orig_obj_row, j] = float(c)
        else:
            # For MAX: apply -c_j storage convention
            tableau[orig_obj_row, j] = -float(c)
...
def solve_steps(objective, constraints, types, goal):
    ...
    # Pass original objective and goal to standardize; it will handle MIN conversion
